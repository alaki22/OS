...
pte_t* superwalk(pagetable_t pagetable, uint64 va, int alloc) {
  // Check if the virtual address is within valid range
  if (va >= MAXVA)
    panic("superwalk: virtual address exceeds MAXVA");
...
void uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += sz){
    ...
    pte_t *superpte = superwalk(pagetable, a, 0);
    if(*superpte == *pte)
      sz = SUPERPGSIZE;
    if(do_free){
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  if(newsz - oldsz < SUPERPGSIZE || hassuperpg() == 0) {
    ...
    if(uvmalloc(pagetable, oldsz, SUPERPGROUNDUP(oldsz), xperm) == 0) return 0;
    oldsz = SUPERPGROUNDUP(oldsz);
    for(a = oldsz; a < newsz; a += sz){
      sz = SUPERPGSIZE;
      mem = superalloc();
      if(mem == 0){
        int allocated = uvmalloc(pagetable, a, newsz, xperm);
        if(allocated == newsz) break;
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += szinc){
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    pte_t *super_pte = superwalk(old, i, 0);
...
void vmprinthelper(pagetable_t pagetable, int level, uint64 va_base){
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    ...
    if(pte & PTE_V){ 
      ...
      uint64 va = va_base + (i<< (12 + 9 * (level-1))); 
      
      
      for(int l = 0; l < 4 - level; l++){
        printf(" ..");
      }
      ...
      if((pte & (PTE_R | PTE_W | PTE_X)) == 0){
        vmprinthelper((pagetable_t)PTE2PA(pte), level - 1, va); 
      }
    }
  }
}
